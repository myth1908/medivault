...
export default async function UsersPage() {
  ...
  // Fetch medical profiles via service role
  let profiles: { user_id: string; blood_type: string | null; organ_donor: boolean; allergies: string[] }[] | null = null
  try {
    const { data } = await adminClient.from('medical_profiles').select('user_id, blood_type, organ_donor, allergies')
    profiles = data
  } catch { profiles = null }

  const profileMap: Record<string, { blood_type: string | null; organ_donor: boolean; allergies: string[] }> =
    Object.fromEntries((profiles ?? []).map(p => [p.user_id, p]))

  interface AdminUser {
    id: string; email: string; full_name: string; created_at: string; role: UserRole
