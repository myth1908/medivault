...
export default async function RolesPage() {
  ...
  if (myRole?.role !== 'superadmin') {
    redirect('/admin?error=superadmin_required')
  }
  ...
  interface AdminUser { id: string; email: string; full_name: string; created_at: string; role: UserRole }
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500">
          Superadmin-only. Assign and manage user roles across the platform.
        </p>
      </div>
        ...
        {users.map(u => (
          <Card key={u.id}>
            <div className="flex items-center gap-4">
              ...
              {/* Current role + changer */}
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${roleVariantClass[u.role as UserRole]}`}>
                  {ROLE_LABELS[u.role]}
                </span>
                {u.id !== currentUser.id && (
                  <RoleChangeButton
